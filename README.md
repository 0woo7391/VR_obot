...
본 프로젝트는 Windows Unity VR 기반의 조종 핸들러 인터페이스와 Ubuntu ROS2 Humble 제어 코어를 연결하여, **두산로봇 E0509(6축 협동 로봇)** 및 **RH-P12-RN-A(1자유도 그리퍼)** 디지털 트윈의 실시간 모니터링 및 모션 플래닝을 중계하는 통합 제어 인프라입니다.
...
sequenceDiagram
    autonumber
    participant Unity as Unity VR (Windows)
...
실행하기 전 터미널에서 작업 공간의 환경 변수를 반드시 로드해야 합니다.
```bash
cd ~/Vrobot_ws
source install/setup.bash
```

### [터미널 1] 로봇 본체 시뮬레이터 및 RViz 구동
```bash
ros2 launch vrobot_description vrobot_full_sim.launch.py
```

### [터미널 2] ROS-TCP 연결 엔드포인트 및 명령 제어 핸들러 구동 (10초 대기 후 권장)
```bash
...
### 기동 시 "Exit code -11" 또는 "Segmentation fault" 에러 발생 시
이전 실행 시 백그라운드 프로세스가 원활히 닫히지 않고 좀비 상태로 메모리를 잡고 있어 일어나는 DDS 충돌 현상입니다. 새 런치를 켜기 전 아래 명령어를 실행하여 찌꺼기 프로세스를 청소해 주십시오.

```bash
killall -9 ros2 rviz2 robot_state_publisher ros2_control_node move_group python3
```
